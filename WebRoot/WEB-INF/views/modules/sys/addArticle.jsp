...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/user/featurePackageList">我的专题</a></li>
		<li class="active"><a href="${ctx}/cms/featurePackage/addArticle?id=${featurePackage.id}&flag=userCenterFeaturePackage">添加知识</a></li>
	</ul>
...
//	        	return false;
			...
			$("#addArticleButton").click(function(){
				$.jBox.open("iframe:${ctx}/cms/featurePackage/articletofeaturepackage?id=${featurePackage.id}&flag=userCenterFeaturePackage", "添加知识",800,400,{iframeScrolling:'no',
			...
			$("#delBatchButton").click(function(){
				...
				$.jBox.confirm('你确定删除被选中的知识吗?','提示',function(v,h,f){
					if(v=='ok'){
						$.ajax({
							type : 'GET',
							url : "${ctx}/cms/featurePackage/delBatch?featuerPackageId=${featurePackage.id}&flag=userCenterFeaturePackage" + "&ids=" + marked,
							async: false,
							dataType: 'text',
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th style="display:none">id</th><th>选择</th><th>已添加知识</th><th>作者</th><th>更新时间</th><th>点击数</th><th>推荐数</th><th>操作</th></tr></thead>
		<tbody>
		<c:forEach items="${articleList}" var="article">
			<tr>
				...
				<td class="line_p">
					...
					<a href="${ctx}/cms/featurePackage/deleteArtileFeaturePackage?featuerPackageId=${featurePackage.id}&aritcleId=${article.id}&&flag=userCenterFeaturePackage" target="mainFrame" >移除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
