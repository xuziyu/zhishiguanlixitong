	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			$("#displaybtn,#selectbtn").click(function(){
				...
				//var selectids=$.session.get('selectids');
				$.jBox.open("iframe:${ctx}/cms/stats/uploaderTree?delflag=1", "选择用户",780,400,{iframeScrolling:'no',
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/cms/stats/uploader">上传者统计</a></li>
		<li ><a href="${ctx}/cms/stats/examer">审核者统计</a></li>
	</ul>
	<form id="uploaderForm" action="${ctx}/cms/stats/uploaderSearcher" method="post" class=" form-search">
		<div>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th>上传者</th><th>所在单位</th><th>所在部门</th><th>原创数量</th><th>转载数量</th><th>文章数量</th>
		<tbody>
		<c:forEach items="${page.list}" var="stats">
			<tr>
				...
				<td><a href="${ctx}/cms/stats/Articlelist?id=${stats.userid}&isChecked=1&beginDate=${stats.beginDate}&endDate=${stats.endDate}">${stats.oricount}</a></td>
				
				<td><a href="${ctx}/cms/stats/Articlelist?id=${stats.userid}&isChecked=2&beginDate=${stats.beginDate}&endDate=${stats.endDate}">${stats.repcount}</a></td>
				
				<td><a href="${ctx}/cms/stats/Articlelist?id=${stats.userid}&isChecked=3&beginDate=${stats.beginDate}&endDate=${stats.endDate}">${stats.arcount}</a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
