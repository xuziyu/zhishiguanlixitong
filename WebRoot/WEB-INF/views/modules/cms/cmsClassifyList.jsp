<%@ page contentType="text/html;charset=UTF-8"%>
  <%@ include file="/WEB-INF/views/modules/cms/front/include/taglib.jsp"%>
    ...
    <head>
      <meta charset="utf-8">
      <title>知识库</title>
      <link rel="stylesheet" href="${ctxStatic}/source-classify/knowledge.css" type="text/css">
      <style type="text/css">
      .abbreviate.off {
        color: #000;
        background: #abd8dd;
      ...
      <script src="${ctxStatic}/source-classify/jquery-1.7.2.min.js" type="text/javascript"></script>
      ...
      function success_ajax(data, searchtext) {
        ...
        if (tt1 != null) {
          for (var i = 0; i < tt1.length; i++) {
            ...
            var examiner = "审批人:";
            var exaname = tt1[i].examiner.name;
            if (exaname) {
              examiner = examiner + exaname;
            }
            ...
            if (searchtext != "") {
              str = str + "<dl onmouseover='dl_mouseover(this)' onmouseout='dl_mouseout(this)'><dt><a href='" + tt1[i].url + "'  target='_blank' class='abbreviate'>" + tt1[i].title + "</a><span class='fl'>上传时间:<i>" + tt1[i].updateDate +
                "</i><i style='margin-left:20px'>" + examiner + "</i></span><div class='fenlei'><i title='" + tt1[i].path + "' class='abbreviate'>" + tt1[i].category.name + "</i>" + hasAttFile + "</div></dt><dd><p><i>" + tt1[i].hits +
      ...
      $(function() {
        ...
        var imageArr = [];
        var imageId = []; 
        <c:forEach items = "${fnc:getCategoryTreeListMap()}"
        var = "list"
        varStatus = "state" >
        ...
        $("#topmenu li").each(function() {
          ...
          for (var i = 0; i < imageId.length; i++) {
            ...
            if (imageId[i] == id) {
              $(x).find("a").css("background", "url(${ctx1}" + imageArr[i].split(";")[0] + ") no-repeat 6px center");
            }
          }
        });
        ...
        function addcolor(a) {
          ...
          for (var i = 0; i < imageId.length; i++) {
            ...
            if (imageId[i] == a) {
              //悬停时,知识库换背景图片
              //$(x).find("a").css("background","url(${ctx1}"+imageArr[i].split(";")[1]+") no-repeat 6px center");
        ...
        $("#topmenu li").on({
          ...
          'mouseleave': function() {
            ...
            for (var i = 0; i < imageId.length; i++) {
              //如果当前li的id(a)和遍历后符合条件的id数组中的元素匹配
              if (imageId[i] == ID) {
                $(this).find("a").css("background", "url(${ctx1}" + imageArr[i].split(";")[0] + ") no-repeat 6px center");
              }
              ...
              success: function(data) {
                ...
                if (tt1 != null) {
                  for (var i = 0; i < tt1.length; i++) {
                    ...
                    if (searchtext) {
                      str = str + "<dl onmouseover='dl_mouseover(this)' onmouseout='dl_mouseout(this)'><dt><a href='" + tt1[i].url.replace(/view/g, "views").replace(/\.html/g, "?search=" + searchtext) + "'  target='_blank' class='abbreviate'>" + tt1[i].title + "</a><span class='fl'>上传时间:<i>" + tt1[i].updateDate +
                        "</i><i style='margin-left:20px'>" + examiner + "</i></span><div class='fenlei'><i title='" + tt1[i].path + "' class='abbreviate'>" + tt1[i].category.name + "</i>" + hasAttFile + "</div></dt><dd><p><i>" + tt1[i].hits +
                        "</i>人已阅读</p><div class='look'><a href='" + tt1[i].url + "' target='_blank' class='scan'>查看阅读</a>" + hasStore + "</div></dd></dl>"
                    } else {
                      str = str + "<dl onmouseover='dl_mouseover(this)' onmouseout='dl_mouseout(this)'><dt><a href='" + tt1[i].url.replace(/view/g, "views").replace(/\.html/g, "?search=" + searchtext) + "'  target='_blank' class='abbreviate'>" + tt1[i].title + "</a><span class='fl'>上传时间:<i>" + tt1[i].updateDate +
              ...
              success: function(data) {
                ...
                if (tt1 != null) {
                  for (var i = 0; i < tt1.length; i++) {
                    ...
                    if (searchtext) {
                      str = str + "<dl onmouseover='dl_mouseover(this)' onmouseout='dl_mouseout(this)'><dt><a href='" + tt1[i].url/*.replace(/view/g, "view").replace(/\.html/g, "?search=" + searchtext)*/ + "'  target='_blank' class='abbreviate'>" + tt1[i].title + "</a><span class='fl'>上传时间:<i>" + tt1[i].updateDate +
                        "</i><i style='margin-left:20px'>" + examiner + "</i></span><div class='fenlei'><i title='" + tt1[i].path + "' class='abbreviate'>" + tt1[i].category.name + "</i>" + hasAttFile + "</div></dt><dd><p><i>" + tt1[i].hits +
                        "</i>人已阅读</p><div class='look'><a href='" + tt1[i].url + "' target='_blank' class='scan'>查看阅读</a>" + hasStore + "</div></dd></dl>"
                    } else {
                      str = str + "<dl onmouseover='dl_mouseover(this)' onmouseout='dl_mouseout(this)'><dt><a href='" + tt1[i].url/*.replace(/view/g, "view").replace(/\.html/g, "?search=" + searchtext)*/ + "'  target='_blank' class='abbreviate'>" + tt1[i].title + "</a><span class='fl'>上传时间:<i>" + tt1[i].updateDate +
    ...
    <body>
      <div class="konw_content knowledge_search">
        <div class="top_content">
          <c:choose>
            <c:when test="${third!='1' }">
              <ul class="top_content_l topmenu" id="topmenu" style="overflow-y: auto; overflow-x: hidden;">
            </c:when>
            <c:otherwise>
              <ul class="top_content_l topmenu" id="topmenu" style="overflow-y: auto; overflow-x: hidden;border:0px solid #FFF;background:#F7F7F7">
            </c:otherwise>
          </c:choose>
          <c:if test="${third!='1' }">
            <c:forEach items="${fnc:getCategoryTreeListMap()}" var="list" varStatus="state">
              <c:if test="${state.index%3==0}">
                <li id="${list.id}">
                  <a href="#" class="li_0 abbreviate" title="${list.name}" id="${list.id}">${list.name}</a>
                  <div class="submenu">
                    <div class="leftdiv">
                        ...
                        <dl>
                          ...
                          <dd>
                            <c:forEach items="${firstlist.secondlist}" var="secondlist">
                              <a href="#" class="abbreviate" title="${secondlist.name}" id="${secondlist.id}">${secondlist.name} </a>
                            </c:forEach>
              ...
              <c:if test="${state.index%3==1}">
                <li id="${list.id}">
                  <a href="#" class="li_1 abbreviate" title="${list.name}" id="${list.id}">${list.name}</a>
                  <div class="submenu">
                    <div class="leftdiv">
                      <c:forEach items="${list.firstlist}" var="firstlist">
                        <dl>
              ...
              <c:if test="${state.index%3==2}">
                <li id="${list.id}">
                  <a href="#" class="li_2 abbreviate" title="${list.name}" id="${list.id}">${list.name}</a>
                  <div class="submenu">
                    <div class="leftdiv">
                      <c:forEach items="${list.firstlist}" var="firstlist">
                        <dl>
          ...
          <div class="top_content_c">
            ...
            <div class="banner">
              <div id="test1">
                <c:forEach items="${fnc:getCategoryTreeListMap()}" var="list" varStatus="state">
                  <div id="submenu${state.index}" class="submenu1" style="overflow-y: auto; overflow-x: hidden">
                          ...
                          <dd>
                            <c:forEach items="${firstlist.secondlist}" var="secondlist">
                              <c:if test="${secondlist.hasArticle>'0'}">
                                <a href="${pageContext.request.contextPath}${fns:getFrontPath()}/categoryid-${secondlist.id}" title="${secondlist.name}" class="abbreviate off">${secondlist.name}
														</a>
                              </c:if>
                              <c:if test="${secondlist.hasArticle=='0'}">
                                <a class="abbreviate">
                  ...
                  <div class="list">
                    <c:if test="${!empty split.pageList}">
                      <c:forEach items="${split.pageList}" var="list">
                        <dl>
                          <dt>
													<a href="${list.url}" target="_blank" class="abbreviate">${list.title}</a><span class='fl'>更新时间:<i><fmt:formatDate
																value="${list.updateDate}" type="both" />
													</i> 
													<i>
														<c:if test="${not empty list.examiner.name}">
															审批人:${list.examiner.name}
														</c:if>
														<c:if test="${empty list.examiner.name }">
															审批人
														</c:if>
													</i>
                          ...
                          <dd>
                            ...
                            <div class="look">
                              <a href="${list.url}" target="_blank" class="scan">查看阅读</a><span <c:if test="${list.articleData.hasStore==0}">
													<span
                    ...
                    <c:if test="${!empty split.pageList}">
                      <div class="fenyemore">
                        ...
                        <c:if test="${split.currentPage!='1'}">
                          <a href='javascript:;' class='prePage' toPage="${split.currentPage}">上一页</a>
                        </c:if>
                        <c:if test="${split.currentPage!=split.pageNum}">
                          <a href='javascript:;' class='nextPage' toPage="${split.currentPage+1}">下一页</a>
                        </c:if>
                        <a href='javascript:;' class='lastPage' toPage="${split.pageNum}">尾页</a> 跳转至
                        <input type='text' name='text' class='npage' onchange="change(this)" value="${split.currentPage}" style="text-align: center" size="1"> 页
                        <a href='javascript:;' class='toPage' toPage="${split.currentPage}">GO</a> 共${split.pageNum}页
                      </div>
                    </c:if>
                    <c:if test="${empty split.pageList}">
                      <div class="fenyemore">
                  ...
                  <div class="list">
                    <c:if test="${!empty split.pageList}">
                      <c:forEach items="${split.pageList}" var="list">
                        <dl>
                          <dt>
													<a href="${list.url}" target="_blank" class="abbreviate">${list.title}</a><span class='fl'>上传时间:<i><fmt:formatDate
																value="${list.updateDate}" type="both" />
													</i> 
													<i>
														<c:if test="${not empty list.examiner.name}">
                        ...
                        <dl>
                          ...
                          <dd>
                            ...
                            <div class="look">
                              ...
                              <c:if test="${list.articleData.hasStore==0}">
                                <span class="love unfavorite" favoriteflag='1' onclick='love_click(this)' markId='${list.id}'>收藏</span>
                              </c:if>
                              <c:if test="${list.articleData.hasStore>0}">
                                <span class="love favorite" favoriteflag='0' onclick='love_click(this)' markId='${list.id}'>收藏</span>
                              </c:if>
          ...
          <c:choose>
            ...
            <c:otherwise>
              <div class="top_content_r" style="border:0px solid #FFF;background:#F7F7F7">
            </c:otherwise>
          </c:choose>

          <c:if test="${third!='1' }">
            <div class="upload_search">
              <a onclick="upload(this);return false" class="upload_article" id="" href="">上传知识</a>
            </div>
            <div class="black">
