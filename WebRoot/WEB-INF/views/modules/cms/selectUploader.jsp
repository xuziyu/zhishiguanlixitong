...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			selectedTree = $.fn.zTree.init($("#selectedTree"), setting, pre_selectedNodes);
			var key=$("#key");
		...
		var pre_selectedNodes =[
   		        <c:forEach items="${selectlist}" var="user">
   		        {id:"${user.id}",
   		         pId:"0",
		...
		var pre_ids = [
		        <c:forEach items="${selectIds}" var="id">
		        "${id}",
		        </c:forEach>];
		var ids = [
		        <c:forEach items="${selectIds}" var="id">
		...
		function treeOnClick(event, treeId, treeNode, clickFlag){
			...
			if("officeTree"==treeId){
				...
				$('.juzhong').show();
				if(delflag=='1'){
					$.get("${ctx}/sys/role/uploaders?officeId=" + treeNode.id, function(userNodes){
					...
					$.get("${ctx}/sys/role/managers?officeId=" + treeNode.id, function(userNodes){
					var userTree_setting={
							view: {selectedMulti:false,nameIsHTML:true,showTitle:true,dblClickExpand:false},
							data: {simpleData: {enable: true},key:{title:"officename"}},
							callback: {onClick: treeOnClick},
		...
		function clearAssign(){
			var submit = function (v, h, f) {
			    if (v == 'ok'){
					...
					ids=[];
			    	//$.fn.zTree.init($("#selectedTree"), setting, pre_selectedNodes);
			    	//$.jBox.tip(tips, 'info');
			    } else if (v == 'cancel'){
			    	// 取消
		...
		function sure(){
			...
			//如果没有在ids中,新选中的额外节点再存放到ids中。
				if($.inArray(String(nodes[i].id),ids)<0){
					...
					nodes[i].name="<font style='font-weight:bold;'>"+nodes[i].name+"</font>";
...
.rb{
	...
	<div id="assignRole" class="row-fluid span12">
		...
		<button class="btn" id="btn" onclick="searchNode()">搜索</button>
			...
			<div class='juzhong' style="text-align:center;"><img src='${ctxStatic}/source-index/images/loading.gif'/><p>正在加载中,请稍候...</p></div>
			<div class="rb">
